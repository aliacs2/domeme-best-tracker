...
def crawl_one(url):
    ...
    for item in items:
        ...
        results.append({
            'rank': rank, 'change': change, 'img': img, 'title': title,
            'price': price, 'qty': qty, 'biz': biz, 'url': f"https://domeggook.com{href}"
        })
    return results
...
def main():
    ...
    dates = json.load(open(index_path, 'r', encoding='utf-8')) if os.path.exists(index_path) else []
    if today not in dates:
        dates.append(today)
        dates.sort(reverse=True)
        json.dump(dates, open(index_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
